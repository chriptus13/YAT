...
class SirenMessageConverter : MappingJackson2HttpMessageConverter() {
    override fun writeInternal(`object`: Any, type: Type?, outputMessage: HttpOutputMessage) {
        super.writeInternal((`object` as OutputModel).toSirenOutputModel(), type, outputMessage)
    }

    override fun canWrite(mediaType: MediaType?) =
            mediaType?.isCompatibleWith(HateoasMediaType.APPLICATION_JSON_SIREN) ?: false
}